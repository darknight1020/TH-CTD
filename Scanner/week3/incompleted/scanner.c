...
void skipComment() { // Loai chu thich (* *)
  int check = 0;
...
Token *ReadString(void)
{
  ...
  while (charCodes[currentChar] != CHAR_QUOTATION ){ // khi chua doc het dau 2 nhay
    if (currentChar == EOF) break;
    
    if (i > MAX_IDENT_LEN){ // Neu dai hon MAX_IDENT_LEN minh dat thi bi loi
  ...
  TokenType tokentype = checkKeyword(token->string);// kiem tra checkKeyword
  if (tokentype != TK_NONE)
    token->tokenType = tokentype;
  else
...
{
  Token *tokenFloat = makeToken(TK_FLOAT, lineNo, colNo);// lay so thuc
  Token *tokenNumber = makeToken(TK_NUMBER, lineNo, colNo);// lay so nguyen
  ...
  if (charCodes[currentChar] == CHAR_PERIOD)//Neu co chua dau cham trong so, nhan so thuc
  {
    tokenFloat->string[count] = (char)currentChar;
    count++;
    readChar();
